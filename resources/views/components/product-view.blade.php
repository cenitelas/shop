<div class="card w-25 m-4 shadow new-card d-flex flex-column justify-content-between" style="height: 350px; cursor: pointer">
    ...
    @if($product->image_path)
        <a class="h-75" href="{{route('products.show',$product)}}">
            <img src="{{ Storage::url($product->image_path) }}" alt="{{ $product->name }}" class="card-img rounded h-100"/>
        </a>
    @endif
            ...
            @auth
                @if(auth()->user()->role->name==='admin')
                    <div class="d-flex align-items-center">
                        <a href={{ route('products.edit',$product) }} class="btn btn-outline-primary p-1 pl-2 pr-2 m-1 h-25">
                        ...
                        <form action="{{ route('products.destroy', $product) }}" method="post">
                            @csrf @method('delete')
                            <button class="btn btn-outline-danger p-1 pl-2 pr-2 m-1">
